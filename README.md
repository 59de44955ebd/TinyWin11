...
2. Use `VhdTool` to append a .vhd footer (just a few extra bytes) to the image file, rename it to `TinyWin11_23H2_English.img.vhd` and mount it in the system (using Explorer or DiskPart).
...
7. Use `Sdelete` (by Sysinternals) to zero the free space (almost 7.3 GB)
...
9. Remove the last 512 bytes (the VHD footer) e.g. with `truncate` and rename the file back to `TinyWin11_23H2_English.img`.
